 ...
 */
function getAdaptiveCollisionRadius(): number {
...
const quantumSymbols: QuantumSymbol[] = [
  {
    ...
    description: '<h3>Artist: Ana (@lunattic_sounds)</h3>' +
      ...
      '<p>The exhibition\'s soundscape blends two compositions: an immersive, sustained soundscape inspired by Pauline Oliveros\' concept of Quantum Listening, built from recordings of quantum experiments and sonified data, and featuring poetry by Glasgow-based poet Nadia Violette. This gradually shifts into a lighter, more melodic improvisation created in collaboration with Glasgow-based sound artist <a href="https://www.instagram.com/ross_yokermoon/" target="_blank">Yoker Moon</a>.</p>' +
      ...
      '<p><strong>Medium:</strong> Sound, Bluetooth speakers made from recycled plastic, home-made hi-fi speakers</p>' +
  ...
  },
  {
    ...
    description: '<h3>Artist: Matthew Wasylko</h3>' +
      ...
      '<p>Connection is a 3D interactive artwork inspired by graphical representations of quantum entanglement. Built using shaders within a 3D graphics framework and run on Soundplay’s Arcade Unit - the piece explores the dynamic, electric-like space between two bound particles. Their connection is in constant motion—shifting, weaving, and responding to semi-random input within mathematical expressions. Emerging patterns cluster, fold, and overlap, forming new, evolving shapes. Within and across the particles, a continuously excited tessellation blends colour, form, and movement. Though separate, the particles remain perpetually linked, flying together through an otherwise empty space.</p>' +
